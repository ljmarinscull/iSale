<template>
  <div class="container mx-auto flex-column p-8 bg-gray-200">
    ...
    <h2 class="w-full my-4 text-3xl font-bold leading-tight text-center text-gray-800">Edit product</h2>
    <ValidationObserver v-slot="{ handleSubmit }" class="flex justify-center items-center">
      <form
      ...
      >
        ...
        <!-- <div class="flex justify-between text-blue-400 items-center text-center mb-10 relative">
          <input
            ...
            accept="image/*"
            class="opacity-0 absolute max-w-md h-8 bg-red-600 cursor-pointer"
          />
...
export default {
  ...
  async created() {
    const value = await this.getProduct(this.$route.params.id);
    if (!value) {
